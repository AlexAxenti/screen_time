...
const WeeklyUsageChart = ({
	...
	const lastWeekScaffold: LastWeekScaffold[] = useMemo(() => {
		...
		for (let i = 0; i < 7; i++) {
			...
			const dayLabel = startOfWeekDate.toLocaleDateString("en-US", {
				weekday: "short",
			});
	...
	const mergedWeeksDailyUsage = useMemo(() => {
		return lastWeekScaffold.map((day) => {
			const match = weeksDailyUsage?.find(
				(usage) => usage.day_start_ms === day.startOfDayMs,
			);
		...
		<Box sx={{ width: "100%", height: "300px" }}>
			<ResponsiveContainer width="100%" height="100%">
				...
				>
					...
					<XAxis type="category" dataKey="dayLabel" />
					<YAxis
						type="number"
						allowDecimals={false}
						label={{
					...
					/>
					<Bar dataKey="totalDurationMs" fill="#1976d2" radius={[6, 6, 0, 0]}>
						{mergedWeeksDailyUsage.map((usage, i) => {
