...
let useGetUsageFragmentation = (startTime: number, endTime: number) => { 
  return useQuery({
    queryKey: ['usage_fragmentation', startTime, endTime],
    queryFn: async () => {
      try {
        let fragmentation: UsageFragmentation[] = await invoke('get_usage_fragmentation', { startTime, endTime })
